...
class Decoder {
 public:
   ...
   *
   * \return True if a frame could be extracted, false if not. If the resulting
   * frame is empty the decoder was fully flushed and no more data is available.
   * Calls to the operator aftewards result in exceptions.
   * */
